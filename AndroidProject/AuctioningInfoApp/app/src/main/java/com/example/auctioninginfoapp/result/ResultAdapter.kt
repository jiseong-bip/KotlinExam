...
/** ResultFragment에서 검색(농산물 공공DB) 결과를 리사이클러뷰에 데이터를 보여주는 어댑터  */
...
class SaveAdpater : PagingDataAdapter<FreshData, ItemViewHolder>(DIFF_CALLBACK) {
    ...
    override fun onBindViewHolder(holder: ItemViewHolder, position: Int) {
        ...
        //GETITEM: FreshData(id=1, saveId=1, mname=사과, sname=후지,...., tradeAmt=1), SaveAdpater
    }
    ...
    fun bindItems(fresh: FreshData?) {
        fresh?.let {
            ...
            binding.txtUnit.text =
                "${fresh.mname} > ${fresh.sname} (등급:${fresh.grade})"
