...
/* 친구들의 목록을 출력하기 위한 어댑터 클래스 선어
    - 리사이클러뷰에 표시될 뷰 홀더와 각 아디템 뷰를 생성하고, 데이터를 바인딩
    - 라사이클러뷰의 어댑터는 RecyclerView.Adapter를 상송하고, <제너릭>타입으로 선언된 뷰홀더를 넣어야함
* */
/** 친구들의 목록을 출력하기 위한 어댑터 클래스 선언
...
class FriendsAdapter(private val itemData: ArrayList<FriendsData>) :
    ...
    /** 뷰 홀더에 데이터 바인딩(bindItems() 함수를 호출)
    ...
    inner class ViewHolder(private val binding: ListItemBinding) : RecyclerView.ViewHolder(binding.root) {
        ...
        fun bindItem(friend: FriendsData) {
            binding.userNameText.text = friend.friendName
            binding.userEngName.text = friend.englishName
